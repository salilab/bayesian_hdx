...
class State(object):
    ...
    def add_dataset(self, dataset):
        for p in dataset.get_peptides():
            if not self.peptide_sequence_consistency(p):
                raise Exception(
                    "Exiting at State.add_dataset. Peptide "
                    + p.sequence + "does not match")
    ...
    def peptide_sequence_consistency(self, peptide):
        ...
        for residue_number in range(peptide.start_residue,
                                    peptide.end_residue):
    ...
    def get_coverage(self, peptides=None):
        ...
        for n in range(len(self.sequence)):
            for p in peptides:
                ...
                # in coverage
                if n >= p.start_res and n < p.end_res-1:
                    self.coverage[n] += 1
    ...
    def calculate_sectors(self, peptides=None):
        ...
        for x in residue_peptide_sets:  # x is residue number in PDB terms
            ...
            self.sectors.append(
                Sector(self, sector_residues, sector_peptide_ids,
                       sector_number))
    ...
    def calculate_score(self):
        ...
        if not self.has_model:
            raise Exception("System.calculate_score: Cannot calculate "
                            "score without a model!!")
        if not self.has_data:
            raise Exception("System.calculate_score: Cannot calculate score "
                            "without data!!")


class Sector(object):
    """ Each Sector object represents a portion of the peptide sequence with
        differential overlap by the MS peptide fragments.
