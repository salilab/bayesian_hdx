"""
   Classes that store the representation and the sampled parameters for each
   state
...
class ResidueGridModel(object):
    '''
    Models the system as individual residues with protection factors along a
    grid
    Defined by a grid_size and the parameters of the datasets included in
    the state
    ...
    @param protection_factors - Boolean. Set to true to calculate protection
           factors. False to calculate rates only.
    '''
    def __init__(self, state, grid_size):
        self.state = deepcopy(state)
        self.length = len(state.get_sequence())
        self.grid_size = grid_size
        self.protection_factors = numpy.zeros(len(state.get_sequence()))
    ...
    def generate_model(self, random=True, value=1, initialize=False):
        ...
        if (not random
                and (type(value) is not int or value > self.grid_size
                     or value < 1)):
            raise Exception(
                "ResidueGridModel.generate_model: Value error. Either allow "
                "random assignment or set an integer value from 1 "
                "to grid_size")

        for i in range(self.length):
            if sequence[i] != "P":
                if random:
                    model[i] = numpy.random.randint(
                        1, high=self.grid_size, size=1)[0]
                else:
                    model[i] = int(value)
    ...
    def convert_model_to_protection_factors(self, model):
        # For a vector of grid values (the model), return a vector
        # of protection values
        pf_grids = self.pf_grids

        self.model_protection_factors = [
            pf_grids[i][int(model[i]-1)] for i in range(self.length)]
        return self.model_protection_factors

    def calculate_protection_factor_grids(self, threshold=0.01):
        '''
        We theorize that the protection factor will be constant over multiple
        experiments. Therefore, the grid of observable protection factors for
        a given residue is dependent on all datasets.

        @param threshold - Means that the grid will start at a protection
               factor value where (1-threshold) * 100%
               will be observed at the first timepoint and end at a protection
               factor value where threshold * 100% of exchange will be
               observed for this site at the longest timepoint.
        '''

        # For each dataset, get the observable protection factors for
        # each residue
        observable_pfs = []
        if not self.state.has_data:
            raise Exception(
                "Cannot calculate protection factor grid because state "
                + self.state.name + " has no associated datasets")
        for d in self.state.get_datasets():
            d.calculate_observable_rate_bounds(threshold)
            observable_pfs.append(d.calculate_observable_protection_factors())

        # The observable range is the largest range over all datasets
        pf_grids = []
        ...
        self.observable_pfs = observable_pfs
        return self.pf_grids

    def get_conversion_grid(self):
        '''
        Returns a list (of length = # of residues) of lists
        (of length = grid_size) with the protection factor values for
        each residue.
        '''
        return self.pf_grids
    ...
    def change_residue(self, residue, value):
        ...
        self.model_protection_factors[residue-1] \
            = self.pf_grids[residue-1][int(value-1)]
    ...
    def get_masked_model(self, observed_residues):
        # get model only returning observed values (0 for everything else)
        mod = []
...
class MultiExponentialModel(object):
    ...
    def guess_init_exp_sequence(self, size_enum):
        ''' Guess initial state by finding most likely exp values for each
            fragment
        ...
        for s in sectors:
            ...
            if (len(sector_frags) == 1
                ...
                #    * sum(overlap) > len(seq) - take (random?) subset of
                #      overlap and assign to sequence
                ...
                #   - assign overlap
                #   - start from smallest fragment and assign values
                #     until you hit n_remaining
                # ------------------
                else:
                    method = "Overlap < num_amides"
                    overlap_sub = numpy.zeros(len(overlap)).astype(int)
                    ...
                    # For each other frag, delete exp_vals closest to
                    # overlap_sub
    ...
    def calc_exp_grid(self, num_bins, first_time_point=10,
        ...
        print("RATE BINS", slowest_rate_bin, fastest_rate_bin,
        ...
        for f in frags:
            for t in f.timepoints:
                ...
                for r in range(f.start_res+2-self.offset,
                    ...
                    model_deut += (1-numpy.exp(-kr*t.time))*self.sat
    ...
    def calculate_marginal_probabilities(self,
        ...
        # Calculates marginal probabilities given grids of omega, sigma0,
        # fmod, and fexp

        self.prob_grid = numpy.zeros(
            (len(self.sigma0_grid), len(self.fmod_grid), len(self.fexp_grid)))
            ...
            for f in range(len(self.fexp_grid)):
                ...
                for s in range(len(self.sigma0_grid)):
                    ...
                    for j in range(1, len(self.omega_grid)):
                        # We're going to calculate the likelihoods
                        # at each point
                        ...
                        if error_model == "truncated_gaussian":
                            ...
                            # use the truncated gaussian factors at
                            # -0.1 and 1.2
                        ...
                        pj = self.gaussian_model(
                            fexp, fmod, omj)*factor*priorj
    ...
    def get_marginalized_sigma_probabilities(self, sigma0,
        ...
        cauchy distribution
        @param delfmod - difference between forward model and noise model in
               delta pct-D units to normalize for fragment length (num amides)
