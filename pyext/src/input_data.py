    ...
    def __init__(self, model, infile, sigma0=1.0):
        ...
        for line in f:
            ...
            if (len(line.split(',')) >= 1 and column_headers is not None
                ...
                state.append(
                    line.split(',')[
                        column_headers.index("sample1_name")].replace(
                            " ", "_"))
                state.append(
                    line.split(',')[
                        column_headers.index("sample2_name")].replace(
                            " ", "_"))
                for s_data in state:
...
class HDXColumns(object):
    ...
    def __init__(self, model, infile, state_name, default_sigma=1.0, offset=0,
        ...
        for line in f.readlines():
            ...
            # If this is not percentD, we want to divide D incorporation
            # by number of amides in fragment and multiply by 100
            if not percentD:
                deut /= float(state_frag.get_num_observable_amides()) * 100

            if state_frag is not None:
                self.add_timepoint_to_frag(state_frag, time, deut,
                                           default_sigma)
    ...
    def add_timepoint_to_frag(self, frag, time, deut, sigma):
        ...
        if deut < 105:  # XXX Hard coded cap on %D value
            ...
            # add the deuteration value.
            # Any other replicate information from the file should be
            # added at this step.
            tp.add_replicate(deut)
            # print len(tp.replicates)
