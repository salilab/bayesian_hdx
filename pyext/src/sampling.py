...
def benchmark(model, sample_sigma):
    ...
    '''Run 100 MC steps and report the time to run 1000 steps
    '''
    print("Running timing benchmark. 10 short simulations will be initiated")
    times=[]
    for i in range(10):
        start=time.time()
...
def do_mc_sampling(model, temp=1, sigma=5.0, error_model="gaussian", NSTEPS=100, save_results=False, 
    ...
    if save_results:
        ...
        for s in model.states:
            ...
            print("HELLO!!!!!")
            print(str(model_exp_sequences[:,:,ns]) + str(model_scores[:,ns]))
...
def exp_model_sector_MC_step(state,exp_model,frags,temp,sigma,error_model="truncated_gaussian", sample_sigma=False):
    ...
    flipped = 0
    for s in state.sectors:
        #print(state.state_name, s.start_res, s.seq, len(s.fragments))
        ...
        for n in range(len(s.seq)):
            ...
            if s.seq[ipick]=='P' or s.seq[ipick]=='p':
                ...
                #otherwise, randomly decide whether to increment or decrement
                    ...
                    delta=-1
                '''
                old = exp_model.exp_seq[resnum]

                exp_model.exp_seq[resnum] = delta #deepcopy(seq0)[resnum] + delta
                new_sector_score=exp_model.calculate_bayesian_score(s.fragments, sigma, error_model)

                # delta score is the new score minus the old. 
                ...
                #print(numpy.exp(-(delscore)/0.01), delscore)
                #print(s.seq, resnum, "||", old, delta, "|STAY|", delscore, flipped, "|", randn, numpy.exp(-(delscore)/0.01))
    #print("Flipped =", i, int(float(flipped)/i*100))
