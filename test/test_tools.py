...
class TestDataset(unittest.TestCase):
    ...
    def test_intrinsic_calculator(self):
        ...
        std_rates = [0., 14.2077, 0.1466, 0.3408, 0.1466, 0.3408, 1.7081, 0.0231]
        ...
        std_rates = [0., 0.6672, 0.2718, 0.2846, 0, 0.0859, 0.4616, 0.2679, 1.3522, 0.7444, 0.0022]
        for i in range(len(rates)):
            self.assertAlmostEqual(rates[i], std_rates[i], places=3)
    ...
    def test_calculate_deut(self):
        ...
        rate = -1
        time = 10
        self.assertAlmostEqual(tools.calculate_deut(rate, time), 0.63212, places=4)


    def test_subsequence_consistency(self):

        sequence = "SANDMAN"
        self.assertTrue(tools.subsequence_consistency(sequence, sequence[2:5], 3))

        self.assertFalse(tools.subsequence_consistency(sequence, sequence[2:5], 2))
