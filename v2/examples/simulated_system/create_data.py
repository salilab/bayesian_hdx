...
pfs = [0,-0.2,-1.2,-3.4,-3.5,-3.8,-5.8,-5.8,-1.5,-1.5] # log rate
...
saturation_est = 1.0        # Deuterium saturation in experiment
sat_pct_error = 0.000001
...
f = open(infile, "w")
f.writelines("peptide_seq,start_res,end_res,time,D_inc,score\n")

for pep in peptides:
  full_deut = []
  for time in range(len(act_timepoints)):
    t = act_timepoints[time]
    deut = 0
    ...
    f.writelines(str(inseq[pep[0]-1:pep[1]])+","+str(pep[0])+","+str(pep[1])+","+str(timepoints[time])+","+str(deut)+",1.0\n")
  print pep, full_deut
